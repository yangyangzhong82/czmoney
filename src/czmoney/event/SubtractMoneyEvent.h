 ...
 * @brief 玩家金额减少前事件 (可取消)
 */
class SubtractMoneyBeforeEvent final : public ll::event::Cancellable<ll::event::Event> {
protected:
    std::string& mPlayerUuid;
    std::string& mCurrencyType;
    int64_t&     mAmountToSubtract; // 要减少的金额
    std::string& mReason1;
    std::string& mReason2;
    std::string& mReason3;

public:
    constexpr explicit SubtractMoneyBeforeEvent(
    ...
    : mPlayerUuid(playerUuid),
      ...
      mAmountToSubtract(amountToSubtract),
      mReason1(reason1),
      mReason2(reason2),
      mReason3(reason3) {}
...
public:
    ...
    int64_t&     getAmountToSubtract() const { return mAmountToSubtract; }
...
public:
    ...
    int64_t const&     getAmountToSubtract() const;
